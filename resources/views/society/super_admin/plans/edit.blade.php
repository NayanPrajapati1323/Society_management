        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <div style="display:flex; gap:.5rem; align-items:center;">
              <input type="number" name="min_units" class="form-control" placeholder="0" value="{{ old('min_units', $plan->min_units) }}" min="0" required />
              <span>to</span>
              <input type="number" name="max_units" class="form-control" placeholder="50" value="{{ old('max_units', $plan->max_units) }}" min="1" required />
            </div>
            <div style="font-size:.75rem;color:var(--muted);margin-top:.25rem;">e.g. 0 to 50, 51 to 100</div>
          </div>
          <div class="form-group">
            <label>Monthly Price (₹) *</label>
            <input type="number" name="monthly_price" id="monthly_price" class="form-control" placeholder="500" value="{{ old('monthly_price', $plan->monthly_price) }}" min="0" step="0.01" required />
          </div>
          <div class="form-group">
            <label>Website Activation Price (One-time ₹) *</label>
            <input type="number" name="website_price" id="website_price" class="form-control" placeholder="1500" value="{{ old('website_price', $plan->website_price) }}" min="0" step="0.01" required />
          </div>
          <div class="form-group">
            <label>Website Maintenance Price (Annual ₹) *</label>
            <input type="number" name="website_maintenance_price" id="website_maintenance_price" class="form-control" placeholder="500" value="{{ old('website_maintenance_price', $plan->website_maintenance_price) }}" min="0" step="0.01" required />
          </div>
          <div class="form-group">
            <label>6 Month Price</label>
            <input type="text" id="six_month_price" class="form-control" readonly style="background:var(--bg-light);" value="{{ number_format($plan->monthly_price * 6, 2, '.', '') }}" />
          </div>
          <div class="form-group">
            <label>12 Month Price</label>
            <input type="text" id="twelve_month_price" class="form-control" readonly style="background:var(--bg-light);" value="{{ number_format($plan->monthly_price * 12, 2, '.', '') }}" />
          </div>
